...
from common.base import sel_end_keys, sel_click, assert_text_in_element, get_text
...
class TestLongin:
    ...
    def test_login(self, test_case, open_page):
        ...
        print(f"📌 执行用例：{case_name}，账号：{name}，密码：{password}")
        try:
            with allure.step(f"输入顾客编号: {name}"):
    ...
    def test_shopping_01(self, DengLu):
        ...
        with allure.step("断言结果故意错误"):
            ...
            assert_text_in_element(driver,(By.XPATH, f"//span[contains(text(),'何思宇 ({ENV.name})')]"),"测试")